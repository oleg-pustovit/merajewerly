...
class SortingFilterComponent extends Component {
  ...
  */
  updateFilterAndSorting(data, event) {
    const facetsFormComp =
      this.closest('facets-form-component') || this.closest('.shopify-section')?.querySelector('facets-form-component');
  
    if (!facetsFormComp || !facetsFormComp.refs.facetsForm) {
      console.error('SortingFilterComponent: Could not find facets form.');
      return;
    }
  
    const actualFormElement = facetsFormComp.refs.facetsForm;
    const checkedInput = this.querySelector('input[name="sort_by"]:checked');
  
    if (!checkedInput) {
      console.error('SortingFilterComponent: Could not find a checked sort_by input.');
      return;
    }
  
    const sortByValue = checkedInput.value;
  
    const tempInput = document.createElement('input');
    tempInput.type = 'hidden';
    tempInput.name = 'sort_by';
    tempInput.value = sortByValue;
    
    actualFormElement.appendChild(tempInput);
  
    facetsFormComp.updateFilters(sortByValue);
  ...
  updateFacetStatus(event) {
    // Перевіряємо, чи існує об'єкт event та його target, щоб уникнути помилок
    ...
    const facetStatus = details.querySelector('.facets__status');
    if (!facetStatus) return;
  
    let selectedOptionName = '';
  
    // Перевіряємо, чи є наш елемент радіокнопкою
    if (target instanceof HTMLInputElement && target.type === 'radio') {
      selectedOptionName = target.dataset.optionName ?? '';
    } else {
      // Якщо це інший тип елемента, виходимо, щоб не було помилок
      return;
    }
  
    // Оновлюємо текст статусу
    if (target.value !== details.dataset.defaultSortBy) {
      facetStatus.textContent = selectedOptionName;
    } else {
      // Якщо обрано значення за замовчуванням, очищуємо статус
