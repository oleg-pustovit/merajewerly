...
export class Slideshow extends Component {
  ...
  async connectedCallback() {
    ...
    // === NEW LOGIC ===
    const setupThumbnailClickEvents = () => {
      const thumbnailsContainer = this.querySelector('.slideshow-controls__thumbnails');

      if (thumbnailsContainer && this.classList.contains('product-media-base-slideshow')) {
        thumbnailsContainer.addEventListener('click', (event) => {
          if (window.innerWidth < 750) {
            return;
          }

          const clickedThumbnail = event.target.closest('.slideshow-control');
          if (!clickedThumbnail) return;

          event.preventDefault();

          const index = this.refs.thumbnails.indexOf(clickedThumbnail);

          if (index > -1) {
            const targetSlide = this.refs.slides[index];
  ...
  async select(input, event, options = {}) {
    ...
    // event?.preventDefault(); // Цей рядок краще залишити в обробнику події, щоб уникнути конфліктів
